...
<<<<<<< HEAD
The genotype frequencies in the population as a whole (accross all generations) can be deduced from the knowledge of the variance of gene frequencies. If an allele has frequency $q$ in a given line, homozygotes of that allele have frequency $q^2$ in that line. The frequency of the homozygotes in the complete population over all lines will be the mean value of q^2 accross all lines. The mean frequency of homozygotes is written as $\bar{q^2}$. The value of $\bar{q^2}$ is obtained by the knowledge of the variance of gene frequencies. In general the variance of a series of observations is obtained by 

\begin{align}
\sigma_q^2 &= \bar{(q^2)} - \bar{q}^2 \notag \\
\bar{(q^2)} &= \bar{q}^2 + \sigma_q^2 
(\#eq:var-inb-bar-q2)
\end{align}

where  $\sigma_q^2$ the variance of gene frequencies among the lines and .$\bar{q}$ is the mean allele frequency. The mean allele frequency is equal to the orginal allele frequency $q_0$. Thus in a population as a whole, the frequency of homozygotes of a particular allele increases and is always in excess by the variance of the gene frequency among the lines. Noting that $\sigma_p^2 = \sigma_q^2$, the genotypic frequencies for a locus with two alleles corresponds to 

```{r genofreq-complete-pop}
tbl_geno_freq <- tibble::tibble(Genotype = c("$A_1A_1$", "$A_1A_2$", "$A_2_A_2"),
                                Frequencies = c("$p_0^2 + \\sigma_q^2$", "$2p_0q_0 - 2\\sigma_q^2$","$q_0^2 + \\sigma_q^2$"))
kntir::kable(tbl_geno_freq,
             booktabs = TRUE,
             longtable = TRUE)
`
...
``
=======
The genotype frequencies in the population as a whole (accross all generations) can be deduced from the knowledge of the variance of gene frequencies. If an allele has frequency $q$ in a particular line, homozygotes of that allele will have frequency $q^2$ in that line. The frequency of homozygotes in the complete population will be the mean value of $q^2$ over all lines. This mean frequency value of homozygotes accross all lines is called $\bar{q^2}$. The value of $\bar{q^2}$ can be found from the knowledge of the variance of gene frequencies among lines. From the general formula of obtaining the variance of a set of observations corresponding to 

\begin{equation}
\sigma_{q}^2 = ($\bar{q^2}$) - \bar{q}^2
(\#eq:var-inv-sigma-Delta-q-def)
\end{equation}
...
(\#eq:var-inv-mean-freq-homozygote)
\end{equation}

where $\bar{q}$ is the mean gene frequency which is the same as the original gene frequency $q_0$. Thus in the complete population, the frequency of the homozygotes of a particular allele increases and is always in excess of the original frequency by an amount equal to the variance of the gene frequency among the lines. In a two-allele system, the same applies to the other allele. The genotypic frequencies for a locus with two alleles can then be summarised as shown in Table \@ref(tab:var-inb-genofreq-tab).

```{r var-inb-genofreq-tab, echo=FALSE}
tbl_genofreq <- tibble::tibble(Genotype = c("$A_1A_1$", "$A_1A_2$", "$A_2A_2$"),
                               Frequency = c("$p^2 + \\sigma_{q}^2$", "$2pq - 2\\sigma_{q}^2$", "$q^2 + \\sigma_{q}^2$"))
knitr::kable(tbl_genofreq, booktabs = TRUE, longtable = TRUE, caption = "Genotype Frequencies in Population as a Whole")  
```

The genotype frequencies shown in Table \@ref(tab:var-inb-genofreq-tab) are no longer in Hardy-Weinberg equilibrium. This change in genotype frequencies is the result of a mechanism which is called the **dispersive process**. The dispersive process is active as soon as the idealised base population is subdivided into single lines. The increase of the frequency of the homozygous genotypes is the source of a phenomenon called **inbreeding depression**. This depression refers to the reduced fitness of individuals in populations with increasing levels of inbreeding. 

Combining the formulas \@ref(eq:var-inv-sigma-Delta-q) and \@ref(eq:var-inv-mean-freq-homozygote) and furthermore dropping the subscript $t$ in $F_t$ leads to 

\begin{equation}
$\bar{q^2}$ = \bar{q_0}^2 + \sigma_{q}^2 = \bar{q_0}^2 + p_0q_0  F
\end{equation}

>>>>>>> 5e307628594d0f50e8b483578e0be6932fa82935

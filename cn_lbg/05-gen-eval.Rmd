...
The same re-arrangment of terms in the basic model is illustrated by Figure \@ref(fig:basicmodelrearrterm)
...
\begin{align}
r_{a,\tilde{y}} &= \frac{cov(a,\tilde{y}) }{\sigma_a \sigma_y} \notag \\
                &= \frac{\sigma_a^2}{\sigma_a \sqrt{(1 + (n-1)t)/n \sigma_y^2}} \notag \\
                &= h \sqrt{n/(1 + (n-1)t)} \notag \\
                &= \sqrt{nh^2/(1 + (n-1)t)} \notag \\
                &= \sqrt{b}
...
where $a_s$ and $a_d$ denote the breeding values of sire $s$ and dam $d$, respectively and $m_k$ and $e_k$ stand for the mendelian sampling and the environmental effect of daughter $k$. Using the same principles as in section \@ref(repeatedrecords), we get
...
assuming there is no environmental covariance between half-sib records and the intra-class correlation $t$ is $\frac{1/4 \sigma_a^2}{\sigma_y^2}$. Then we can compute the regression coefficient as 
...
The term for $r_{a, \bar{y}}$ in \@ref(eq:accuracyprogeny) approaches $1$ as the number of progeny increases. The reliability ($r_{a, \bar{y}}$) of the predicted breeding value is $n/(n+k)$ and corresponds to $1/2 * b$ computed in \@ref(eq:regcoeffprogeny). 

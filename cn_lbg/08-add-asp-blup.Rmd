...
r_{u,\hat{u}} = \frac{cov(u, \hat{u})}{\sqrt{var(u) * var(\hat{u})}} 
              = \frac{var(\hat{u})}{\sqrt{var(u) * var(\hat{u})}} 
...
The vector $s$ of unknowns in the mixed model equations consists of the vector $\hat{\beta}$ of estimates of fixed effects and the vector $\hat{a}$ of predicted breeding values, which means

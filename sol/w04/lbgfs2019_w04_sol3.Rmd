...
tbl_genotypic_decomp <- tibble::tibble(Genotype             = c("$G_1G_1$", "$G_1G_2$", "$G_2G_2$"),
                                       `Genotypic Value`    = c("$a = `r a`$", "$d = `r d`$", "$-a = `r -a`$"),
                                       `Breeding Value`     = c("$2q\\alpha = `r 2*q*alpha`$", "$(q-p)\\alpha = `r (q-p) * alpha`$", "$-2p\\alpha = `r -2*p*alpha`$"),
                                       `Dominance Deviation`= c("$-2q^2d = `r -2*q^2*d`$", "$2pqd = `r 2*p*q*d`$", "$-2p^2d = `r -2*p^2*d`$"))

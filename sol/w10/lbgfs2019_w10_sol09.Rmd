...
## Assumptions
* Although, cattle does not follow the same mode of inheritance as the organism shown in the lecture, the computations as shown in the lecture can be used as an approximation. * The ratio between male and female animals can be assumed to be $`r n_sex_ratio`$.
...
\begin{equation}
F_t = 1 - (1 - `r round(n_delta_F, ndigits = 4)`)^{`r n_generations`} = `r round(n_inbreeding_coefficient, digits = 4)`
(\#eq:inbreeding-coefficient)
\end{equation}
...
Use the same assumptions as in Problem 1 and compute the inbreeding depression at two different genetic loci.

a. Locus $A$ is purely additive, hence the genotypic value of the heterozygous genotype is in the middle between the values of the two homozygous genotypes. In other words, the quantity $d = 0$. The minor allele frequency (MAF) of the positive allele of locus $A$ is $0.1$
b. 
